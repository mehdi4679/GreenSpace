...
function defaultid2(id) {
    
    data = {};
    data.cl = {};
    data.cl.CID = id;
    LoadService("Catalog.aspx/Default2", data, function (data, status) {
        if (data.d != null) {
...
function OnSuccess(response) {
    ...
    $.each(Tbl_student, function () {
        ...
        $("td", row).eq(3).html('<a class="AEdit" title="Edit record." href=#' + $(this).find("CaID").text() + ' onclick="SetData(' + $(this).find("CaID").text() + ')" />'+
'<a class="ADelete"  href=#' + $(this).find("CaID").text() + ' title="delete." onclick="del(' + $(this).find("CaID").text() + ')" />' +
'<a class="' + $(this).find("atrue").text() + '" href=#' + $(this).find("CaID").text() + ' title="پیش فرض" onclick="defaultid(' + $(this).find("CaID").text() + ')" />'+
'<a class="' + $(this).find("atrue2").text() + '" href=#' + $(this).find("CaID").text() + ' title="حذف منطقی" onclick="defaultid2(' + $(this).find("CaID").text() + ')" />');
